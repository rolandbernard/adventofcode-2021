...
uint64_t maxNumber(unordered_set<AluState>& invalid, AluState state, const Program* prog, uint64_t number) {
    if (state.pc >= prog->length) {
        ...
        if (invalid.find(state) == invalid.end()) {
            for (int64_t i = 9; i >= 1; i--) {
                size_t max = maxNumber(invalid, runProgram(state, prog, true, i), prog, number * 10 + i);
                if (max != 0) {
                    return max;
                }
            }
...
int main() {
    ...
    int64_t number = maxNumber(invalid, initial, prog, 0);
    free(prog);
