...
applySteps chart 0 = (chart, 0)
applySteps chart n = (stepped, flashes + flashed)
    where
        (stepped, flashes) = applyStep prev
        (prev, flashed) = applySteps chart (n - 1)

main = do
    dat <- readFile "input.txt"
    let chart = map (map (\x -> ord x - ord '0')) (split (=='\n') dat)
    let (_, flashes) = applySteps chart 100
    putStrLn ("Result: " ++ show flashes)
