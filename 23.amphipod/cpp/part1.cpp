...
static const int rooms[] = {2, 4, 6, 8};

struct GameState {
    int heur;
    int dist;
    array<int, 2> loc[TYPES][DEPTH];
    ...
    for (int i = 0; i < TYPES; i++) {
        for (int j = 0; j < DEPTH; j++) {
            h += costs[i] * (
                ...
                + (s.loc[i][j][0] != rooms[i] ? s.loc[i][j][1] + 1
                    : (s.loc[i][j][1] == 0 ? 1 : 0))
            );
        }
    }
...
bool canMove(const GameState& s, const array<int, 2>& from, const array<int, 2>& to) {
    if (to[1] == 0) {
        for (int i = 0; i < TYPES; i++) {
            if (to[0] == rooms[i]) {
                return false;
...
GameState readInput() {
    ...
    while (cin >> c) {
        if (c >= 'A' && c <= 'D') {
            ...
            state.loc[c - 'A'][is[c - 'A']][1] = 1 + (i / TYPES);
            is[c - 'A']++; i++;
        }
    }
    state.norm();
...
int getBestCost(GameState& initial) {
    ...
    auto try_adding = [&queue, &cost](const GameState& s, int i, int j, const array<int, 2>& to) {
