...
size_t countPaths(Map& map, vector<int>& visited, int at, int end) {
    if (at == end) {
        return 1;
    } else if (map.is_small[at] && visited[at] > 0) {
        return 0;
    } else {
        size_t ret = 0;
        visited[at]++;
        for (int con : map.connections[at]) {
            ret += countPaths(map, visited, con, end);
        }
        visited[at]--;
        return ret;
    }
...
int main() {
    ...
    size_t res = countPaths(map, visited, start, end);
