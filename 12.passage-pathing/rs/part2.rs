...
fn count_paths(map: &mut Map, visited: &mut Vec<bool>, at: usize, allow: bool, start: usize, end: usize) -> usize {
    if at == end {
        return 1;
    } else if (at == start || !allow) && map.small[at] && visited[at] {
        return 0;
    } else {
        let dup = map.small[at] && visited[at];
        let mut ret = 0;
        visited[at] = true;
        for i in map.connections[at].clone().iter() {
            ret += count_paths(map, visited, *i, allow && !dup, start, end);
        }
        visited[at] = dup;
        return ret;
    }
...
fn main() {
    ...
    let count = count_paths(&mut map, &mut visited, start, true, start, end);
