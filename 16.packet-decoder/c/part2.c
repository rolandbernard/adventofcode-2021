...
long computeResult(Packet* packet) {
    if (packet->type == 4) {
        return packet->literal;
    } else {
        long result = packet->type == 1 ? 1 : 0;
        for (size_t i = 0; i < packet->sub_count; i++) {
            long sub = computeResult(packet->subs + i);
            if (packet->type == 0) {
                result += sub;
            } else if (packet->type == 1) {
                result *= sub;
            } else if (packet->type == 2) {
                result = i == 0 || sub < result ? sub : result;
            } else if (packet->type == 3) {
                result = i == 0 || sub > result ? sub : result;
            } else if (packet->type == 5) {
                result = i == 0  ? sub : (result > sub ? 1 : 0);
